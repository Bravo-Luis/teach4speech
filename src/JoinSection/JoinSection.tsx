...
function JoinSection({ webSocket, setWebSocket }: { webSocket: WebSocket | null, setWebSocket: (any: any) => void }) {
    ...
    const isUsernameValid = (username : string, badWords : string[]) => {
        const lowerCaseUsername = username.toLowerCase();
        return !badWords.some(badWord => lowerCaseUsername.includes(badWord.toLowerCase()));
    };
    ...
    const handleJoinGame = () => {
        ...
        if (!isUsernameValid(username, badwords)) {
            ...
            <TextField 
                ...
                onChange={(e) => {
                    if (e.target.value.length <= 6){
                        setGameCode(e.target.value.toUpperCase().trim());
                    }
                }}
                sx={{ 
                    input: { 
